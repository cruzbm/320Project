...
public:
	billboard()
	{
		position = XMFLOAT3((rand()%48-48)/2.0,0,(rand()%33+2)/2.0);
		scale = 1;
		transparency = 1;
	}
	XMFLOAT3 position;
...
public:
	...
	void animation(bitmap *leveldata, float elapsed_microseconds)
	{
		XMMATRIX R, T;
		...
		XMFLOAT3 col = position;

		if (w)
		{
			col.x -= forward.x * speed;
			col.y -= forward.y * speed;
			col.z -= forward.z * speed;
		...
		UINT bitx = -col.x / 2 + 50.5;
		UINT bitz = -col.z / 2 + 0.5;
		BYTE colorPixel = leveldata->get_pixel(bitx, bitz, 0);
		if (colorPixel == 0)
		{
			position = col; 
		}

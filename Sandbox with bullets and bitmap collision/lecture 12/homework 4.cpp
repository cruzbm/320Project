...
ID3D11ShaderResourceView*           g_pTargetA = NULL;
ID3D11ShaderResourceView*           g_pTargetD = NULL;
ID3D11ShaderResourceView*           g_pTargetB = NULL;
...
int target2Active = 0;
int target3Active = 0;
float target1Timer[1][2];
//target1Timer[1][0] = 0.0;
//target1Timer[1][1] = 1.0;
//Use 2d array to tracker float timer in column 1 [1][0], and timer flag for forward and reverse time [1][1]
//Double check use of 2D array
...
HRESULT InitDevice()
{
    ...
    // Load the Texture
    hr = D3DX11CreateShaderResourceViewFromFile( g_pd3dDevice, L"bull.png", NULL, NULL, &g_pTextureRV, NULL );
    if( FAILED( hr ) )
        return hr;

	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"targeta.jpg", NULL, NULL, &g_pTargetA, NULL);
	if (FAILED(hr))
		return hr;

	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"targetd.jpg", NULL, NULL, &g_pTargetD, NULL);
	if (FAILED(hr))
		return hr;
	hr = D3DX11CreateShaderResourceViewFromFile(g_pd3dDevice, L"targetb.jpg", NULL, NULL, &g_pTargetB, NULL);
	if (FAILED(hr))
		return hr;
	...
	t2x = rand() % 22 - 20;
	t3x = rand() % 22 - 20;
	t1z = rand() % 34 + 20;//bmod: furthest target
	t2z = rand() % 20 + 10;//dog: mid range
	t3z = rand() % 10 + 2;//able: closest
...
void CleanupDevice()
{
	...
	if (g_pTargetA) g_pTargetA->Release();
	if (g_pTargetB) g_pTargetB->Release();
	if (g_pTargetD) g_pTargetD->Release();
...
UINT offset = 0;
	...
	cam.animation(level1.get_bitmap(), elapsed);//pass the bitmap to the camera *NEW*
	...
	//TargetA
	static billboard targetA;
	worldmatrix = targetA.get_matrix(view);	
	...
	g_pImmediateContext->PSSetShaderResources( 0, 1, &g_pTargetA);
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer, &stride, &offset);
	...
	static billboard target[5];


	/*if (target1Timer < 5.0) {
		target1Timer += 0.01;
	}
	else if (target1Timer >= 0.0) {
		target1Timer -=
	}*/
	...
	if (target1Active == 0) {
		/////// target 1
		target[0].position.x = t1x;
		target[0].position.y = 0.0f;
		target[0].position.z = t1z;
		worldmatrix = target[0].get_matrix(view);
		constantbuffer.World = XMMatrixTranspose(worldmatrix);
		constantbuffer.View = XMMatrixTranspose(view);
		constantbuffer.Projection = XMMatrixTranspose(g_Projection);
		...
		g_pImmediateContext->PSSetShaderResources(0, 1, &g_pTargetB);
		g_pImmediateContext->Draw(12, 0);
	}
	...
	if (target2Active == 0) {
		/////// target 2
		target[1].position.x = t2x;
		target[1].position.y = 0.0f;
		target[1].position.z = t2z;
		worldmatrix = target[1].get_matrix(view);
		constantbuffer.World = XMMatrixTranspose(worldmatrix);
		constantbuffer.View = XMMatrixTranspose(view);
		constantbuffer.Projection = XMMatrixTranspose(g_Projection);
		g_pImmediateContext->UpdateSubresource(g_pCBuffer, 0, 0, &constantbuffer, 0, 0);
		...
		g_pImmediateContext->PSSetShaderResources(0, 1, &g_pTargetD);
		g_pImmediateContext->Draw(12, 0);
	}
	...
	//Toggle Target Appearance
	if (target3Active == 0) {
		/////// target 3
		target[2].position.x = t3x;
		target[2].position.y = 0.0f;
		target[2].position.z = t3z;
		worldmatrix = target[2].get_matrix(view);
		constantbuffer.World = XMMatrixTranspose(worldmatrix);
		constantbuffer.View = XMMatrixTranspose(view);
		constantbuffer.Projection = XMMatrixTranspose(g_Projection);
		g_pImmediateContext->UpdateSubresource(g_pCBuffer, 0, 0, &constantbuffer, 0, 0);
	...
	//Draw TargetD
/*
	static billboard targetD;
	worldmatrix = targetD.get_matrix(view);
	...
	//Target Collision detection - changes active
